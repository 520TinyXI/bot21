...
@register("huanju", "kjqwdw", "欢乐21点", "1.0.1", "https://github.com/kjqwer/astrbot_plugin_game_sy")  
class HuanJuPlugin(Star):
    def __init__(self, context: Context):
        ...
        self.player_names = {}  # 存储玩家ID和名称的映射
        
    @filter.command("创建21点")
    async def create_game(self, event: AstrMessageEvent):
        ...
        yield event.plain_result("21点游戏房间创建成功！请使用 /加入21点 加入游戏，2-4人即可开始。")

    @filter.command("加入21点")
    async def join_game(self, event: AstrMessageEvent):
        ...
        if group_id not in self.game_rooms:
            yield event.plain_result("当前群未创建牌局！请使用 /创建21点 创建")
            return
        ...
        # 保存玩家名称，优先使用昵称，如果没有则使用ID
        player_name = event.get_sender_name() or player_id
        ...
        if len(room["players"]) >= 2:
            yield event.plain_result("人数已达2人，可以使用 /开始21点 开始游戏")

    @filter.command("添加机器人")
    ...
    async def start_game(self, event: AstrMessageEvent):
        ...
        if str(first_player).startswith("bot_"):
            ...
            yield event.plain_result(f"游戏开始！请 @{first_player} ({display_name}) 选择 /要牌 或 /停牌")
        ...
        if points > 21:
            ...
            if next_player:
                ...
                if str(next_player).startswith("bot_"):
                    ...
                    yield event.plain_result(f"轮到 @{next_player} ({display_name}) 的回合，请选择 /要牌 或 /停牌")
            else:
                # 游戏结束，计算结果
            ...
            yield event.plain_result(f"请选择 /要牌 继续要牌 或 /停牌 停牌")
    ...
    async def quit_game(self, event: AstrMessageEvent):
        ...
        if player_id == room["creator"]:
            del self.game_rooms[group_id]
            yield event.plain_result("房主已退出，游戏结束！")
            return
        ...
        if room["current_player"] == player_id:
            players = list(room["players"])
            current_index = players.index(player_id)
            next_player = players[(current_index + 1) % len(players)] if players else None
            ...
            yield event.plain_result("你已退出游戏！")
    ...
    async def next_turn(self, event: AstrMessageEvent, group_id: str, current_player: str):
        ...
        if next_player:
            ...
            if str(next_player).startswith("bot_"):
                ...
                display_name = self.get_player_display_name(next_player)
                msg = MessageChain().message(f"轮到 @{next_player} ({display_name}) 的回合，请选择 /要牌 或 /停牌")
                await self.context.send_message(event.unified_msg_origin, msg)
        else:
            # 游戏结束，计算结果
            result = await self.get_game_result(room)
    ...
    async def get_game_result(self, room: Dict) -> str:
        ...
        if winners:
            winner_names = [self.get_player_display_name(w) for w in winners]
    ...
    def get_player_display_name(self, player_id: str) -> str:
        '''获取玩家显示名称'''
        if str(player_id).startswith("bot_"):
            return self.game_rooms.get(player_id, {}).get("bot_names", {}).get(player_id, "机器人")
        return self.player_names.get(player_id, player_id)

    @filter.command("21点帮助")
